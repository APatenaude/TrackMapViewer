...
A mobile-first deep-zoom race track map viewer for HPDE instructors to share with students. Displays a high-res track photo as a tiled background with toggleable SVG vector overlays — racing line, braking zones, apexes, corner numbers, and a wet line. Installable as a PWA, works offline after first view, and self-hosts via Docker.

![Panel showing layer toggles over a race track map](https://placehold.co/800x400?text=Track+Map+Viewer)
...
The repo ships with placeholder assets (a 100×64px dummy image and demo SVG layers) so the app runs immediately.
...
2. **Update `public/config.json`** — set the pixel dimensions, the SVG/tile paths, the localized title + track name, and your layers (ids must match the SVG `<g>` ids):
   ...
   ```json
   {
     ...
     "trackName": "My Track",
     "layers": [
       { "id": "racing_line", "label": { "en": "Driving line", "fr": "Ligne de conduite" }, "defaultOn": true, "defaultOpacity": 1.0 }
     ]
   }
   ```
...
| `npm run tile` | Generate DZI tiles from `assets/track.jpg` (requires vips) |
...
The published image bundles the Mont-Tremblant map. To host a *different* track, swap the assets (see [Using Your Own Track Assets](#using-your-own-track-assets)) and build from source — the Dockerfile runs the whole pipeline (libvips tiling + Vite build) in one step:
